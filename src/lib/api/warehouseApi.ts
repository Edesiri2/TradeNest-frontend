...
export const warehouseAPI = {
  // Get all warehouses
  getWarehouses: (params?: { search?: string; page?: number; limit?: number; active?: boolean }) => {
    ...
    const endpoint = `/warehouses${queryString ? `?${queryString}` : ''}`;
  ...
  getWarehouseOutlets: (id: string) => {
    return apiRequest(`/warehouses/${id}/outlets`);
  },
};
...
export const outletAPI = {
  // Get all outlets
  getOutlets: (params?: { search?: string; type?: string; warehouse?: string; active?: boolean }) => {
    const queryParams = new URLSearchParams();
    if (params?.search) queryParams.append('search', params.search);
